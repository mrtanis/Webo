...
+ (void)chooseRootVC:(UIWindow *)keyWindow quickLaunchType:(MRTQuickLaunchType)type
{
        ...
        NSLog(@"直接创建新的rootViewController");
        if (type > 0) {
            NSLog(@"直接创建新的rootViewController快捷进入tabBar");
            MRTTabBarController *tabBarVc = [[MRTTabBarController alloc] initFromQuickLaunchType:type];
            keyWindow.rootViewController = tabBarVc;
        } else {
            NSLog(@"直接创建新的rootViewController并选择控制器");
