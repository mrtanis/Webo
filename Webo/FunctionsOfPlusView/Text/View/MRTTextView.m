...
- (void)textChange
{
    ...
    if ([_delegate_mrt respondsToSelector:@selector(textChanged)]) {
        [_delegate_mrt textChanged];
    }
    ...
    {
        NSLog(@"textChange第一种情况");
        rect.origin.y = ceil(textSize.height) + 30;
        self.overview.frame = rect;
        if (self.overview.y + self.overview.height > self.height - 64) {
            self.contentInset = UIEdgeInsetsMake(0, 0, self.overview.height + 30, 0);
        } else {
            self.contentInset = UIEdgeInsetsMake(0, 0, 30, 0);
        }
        
    } else {
        NSLog(@"textChange第二种情况");
        rect.origin.y = boundaryHeight;
        self.overview.frame = rect;
    }
    NSLog(@"self.y:%f,self.height:%f,offset.y:%f", self.y, self.height, self.contentOffset.y);
    NSLog(@"overview.y:%f,overview.height:%f,total:%f", self.overview.y, self.overview.height,self.overview.y + self.overview.height);
...
{
    _overview = overview;
    if (overview.y + overview.height > self.height - 64) {
        self.contentInset = UIEdgeInsetsMake(0, 0, overview.height + 40, 0);
    }
...
- (void)mrt_paste:(UIMenuController *)menuController
{
    ...
    //CGSize textSize = attText.size;
    //self.contentOffset = CGPointMake(0, - textSize.height + self.contentOffset.y);
    /*} else {
