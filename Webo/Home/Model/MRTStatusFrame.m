...
- (void)setUpOriginalViewFrame
{
    ...
    CGRect rect = [self.status.attrText boundingRectWithSize:CGSizeMake(text_Width - 16, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];
    //[textView sizeThatFits:CGSizeMake(textView.bounds.size.width, CGFLOAT_MAX)];
    CGSize contentSize = rect.size;
    
    _originalTextFrame = CGRectMake(text_X, text_Y, text_Width, ceil(contentSize.height + 16));
...
- (void)setUpRetweetViewFrame
{
    ...
    //[textView sizeThatFits:CGSizeMake(textView.bounds.size.width, CGFLOAT_MAX)];
    CGRect rect = [self.status.retweeted_status.attrText boundingRectWithSize:CGSizeMake(text_Width - 16, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];
    CGSize contentSize = rect.size;

    
    _retweetTextFrame = CGRectMake(text_X, text_Y, text_Width, ceil(contentSize.height + 16));
