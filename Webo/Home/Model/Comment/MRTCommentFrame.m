...
- (void)setUpCommentViewFrame
{
    ...
    CGFloat text_Y = CGRectGetMaxY(_commentIconFrame) + MRTStatusCellMargin;
    CGFloat text_Width = MRTScreen_Width - 3 * MRTStatusCellMargin - icon_W_H;
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    
    textAttrs[NSFontAttributeName] = MRTCommentTextFont;
    
    CGRect text_Rect = [self.comment.text boundingRectWithSize:CGSizeMake(text_Width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:textAttrs context:nil];
    
    _commentTextFrame = CGRectMake(text_X, text_Y, ceil(text_Rect.size.width), ceil(text_Rect.size.height));
