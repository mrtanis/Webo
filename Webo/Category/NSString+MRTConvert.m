...
+ (NSString *)videoUrlFromString:(NSString *)htmlStr
{
    ...
    if (range1begin.location != NSNotFound) {
        ...
        //A new string with the percent-encoded sequences removed
        //转换诸如:%3A  %2F  %3F  %26之类的url编码
        videoStr = [videoStr stringByRemovingPercentEncoding];
    ...
    if (range2begin.location != NSNotFound) {
        ...
        NSRange range2realEnd = [substr rangeOfString:@"\"," options:NSLiteralSearch];
        //NSLog(@"range2realEnd(%ld,%ld)", range2realEnd.location, range2realEnd.length);
        if (range2realEnd.location != NSNotFound) {
            videoStr = [substr substringWithRange:NSMakeRange(0, range2realEnd.location + range2realEnd.length - 1)];
        }
