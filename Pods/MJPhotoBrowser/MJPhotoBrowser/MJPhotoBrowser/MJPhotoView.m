...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        _imageView.animatedImage = _photo.animatedImage;
        ...
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.placeholder options:SDWebImageRetryFailed| SDWebImageLowPriority| SDWebImageHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            ...
            [_self photoDidFinishLoadWithAnimatedImage:_imageView.animatedImage image:image];
        }];
        /*
        [SDWebImageManager.sharedManager loadImageWithURL:_photo.url options:SDWebImageRetryFailed| SDWebImageLowPriority| SDWebImageHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            ...
            __imageView.animatedImage = gifImage;
...
#pragma mark 加载完毕
- (void)photoDidFinishLoadWithAnimatedImage:(FLAnimatedImage *)animatedImage image:(UIImage *)image
{
    if (_imageView.image) {
        self.scrollEnabled = YES;
        _photo.animatedImage = animatedImage;
        _photo.image = image;
        [_photoLoadingView removeFromSuperview];
